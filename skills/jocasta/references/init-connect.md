...
5. **Replace the placeholders.** Two files carry one each. `jocasta.yaml` contains `team: PLACEHOLDER_TEAM`; replace `PLACEHOLDER_TEAM` with the confirmed team name, in that file only. `.github/chainguard/jocasta.sts.yaml` contains `PLACEHOLDER_REPO` in its `subject` line; replace it with `<org>/<name>` as GitHub spells it, read back with `gh repo view <org>/<name> --json nameWithOwner -q .nameWithOwner` rather than as typed, because the policy is an exact match against the run's identity and a case difference makes every exchange fail quietly. That file only. Then confirm nothing else carries a marker: `grep -rn PLACEHOLDER_ <workdir>` must print nothing. Leave `schema_version` and `machinery_ref` as the template has them.
...
7. **Offer the OctoSTS App.** Optional, and not something `init` does itself: installing a GitHub App is a browser step. Tell the user that the registry's `validate` and `stale-sweep` workflows exchange their identity for a GitHub App token that lives for minutes whenever the [OctoSTS App](https://github.com/apps/octo-sts) is installed on the new repo, governed by the trust policy pushed in step 6, and give the link: <https://github.com/apps/octo-sts> (install it on `<org>/<name>` only). What it buys: pull requests the stale sweep opens start the `pull_request` workflows (`consensus-merge`, `validate`) the moment they open, and they open even when step 2 was refused; with an organization-level trust policy the same exchange can also reach sources in private repositories, which the repository-level policy the template ships does not. Without the App everything runs on the workflow token. Do not wait for the install; continue.
...
9. **Seed the snapshot** so the first `search` is instant: `gh repo clone <org>/<name> ~/.cache/jocasta/<org>/<name> -- --depth 1`. Skip silently if it already exists.

10. **Close with the plain statement**, then at most one sentence of register. For example:
   ...
   If step 2 was refused, the statement adds one sentence saying so and naming the two remedies: an admin turns on "Allow GitHub Actions to create and approve pull requests", or the team installs the OctoSTS App from step 7.

What `init` does not do: it does not create GitHub teams, branch protection, secrets, or environments; beyond the one Actions setting in step 2, it does not enable or configure Actions past what the workflow files themselves declare; it does not install the OctoSTS App (step 7 only offers the link); it does not add collaborators. The workflow token alone does not cover every template workflow: `validate` and `stale-sweep` run with the repository's own `GITHUB_TOKEN`, which can read only the registry itself, can open pull requests only while the step 2 setting is on, and cannot start other workflows. When they need more they do not read a stored secret (charter D-10): each run exchanges its OIDC identity for a GitHub App token that lives for minutes, through OctoSTS, under the trust policy `init` committed in `.github/chainguard/jocasta.sts.yaml` (`contents` and `pull_requests` write on the registry, for runs on `main`), and falls back to the workflow token when the exchange fails. A caller who is not an admin of the new repo (step 2 refused) gets the sweep's pull requests this way once the App is installed. A personal access token stored as a repository secret is discouraged, not a documented route: it is a standing credential with write access to the registry, and no step here creates one. Anyone who can push to the repo can use the registry; that is GitHub's permission model doing the work (charter N-5).
...
2. **Write the local config** exactly as in `init` step 8, with the same warning if an existing config pointed elsewhere.

3. **Seed the snapshot** as in `init` step 9.
