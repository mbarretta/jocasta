...
The instance's `validate` workflow runs `validate.py --changed-only <ref> --actor <login> --event <event>` on every push to the default branch, with `<ref>` the commit the branch was at before the push (`github.event.before`), `<login>` the GitHub account that pushed, and `<event>` the workflow's `github.event_name`. Besides the schema, it applies the `authorization` rule from `references/schema.md`: between the two commits, under every key of `adoption.yaml`, the only login that may appear or disappear is the actor's own, and any `entries/*.md` the push added must name the actor as owner while any it modified must have been the actor's at `<ref>`. A violation is one line, `adoption.yaml: authorization: <detail>; open a PR instead`, and the workflow goes red. If the skill followed the steps above, that line is not reachable: it exists for a hand edit that touched someone else's login.

Where the rule's reach ends, so nobody mistakes it for more than it is:

- **Pull requests are not authorized by this rule.** On `pull_request` events the workflow checks out GitHub's synthetic merge commit and passes `--event pull_request`, so the check is skipped with a notice; the PR path is gated by review and the consensus action instead (charter D-3, P-5). The same skip applies to pushes made by `github-actions[bot]`, which is how a consensus merge lands. A consequence: on a `synchronize` event GitHub sets `before` to the previous head of the PR branch, so a diff against it would see only the newly pushed commits; that does not matter here because the PR path is skipped by construction.
...
- **A merge commit is not a pass.** On a `push` event the check diffs `<ref>...HEAD` whether or not `HEAD` has two parents. Someone with push access who wraps an edit to another person's entry, or to another login in `adoption.yaml`, in a local `git merge --no-ff`, or who opens a PR for it and clicks Merge themselves, turns the run red as the pusher of that merge. Neither is undone by the check, and the PR's own run passes by construction, so requiring the `validate` status check alone does not stop a self-merge from landing: branch protection on the default branch must also require pull requests and at least one approving review. Without `--event` (a workflow on an older template release calling an older action) the validator falls back to skipping any two-parent `HEAD`, which is a shape test a local merge passes; the current action always passes the event.
