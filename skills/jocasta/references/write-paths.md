...
- reads the entry's owner from the default branch; an entry that is unowned (`owner: ~`) has no owner, and an entry that is not on the default branch at all is new and is refused, whatever the votes: a new entry is registered by its owner with a direct commit;
- merges only one of two content shapes, compared field by field between the default branch and the PR head: a **deprecation** (`status` goes `active` -> `deprecated`, a `deprecated` block appears whose `route` is `owner`, `consensus`, or `stale-source`, every other field is unchanged, and the body is unchanged or has text appended) or a **claim** (`owner` becomes the PR author's login and nothing else changes); any other change gets one comment naming the changed fields and is not merged, even with the owner's approval;
- counts only repository insiders as voices: a reviewer or author is a voice only when GitHub reports their `author_association` as `OWNER`, `MEMBER`, or `COLLABORATOR`; `[bot]` accounts and outside accounts (`NONE`, `CONTRIBUTOR`, ...) are ignored;
...
One trigger caveat for the `stale-source` row. A PR that `stale_sweep.py` opens with the workflow's own `GITHUB_TOKEN` does not start the `consensus-merge` workflow's `opened` run, because GitHub does not let a workflow token trigger further workflows; the same goes for the instance's `validate` run on that PR. The script first runs on the first human event on the PR: a review, a label, or a push to its branch. A team that wants the sweep's PRs treated like a person's from the moment they open installs the OctoSTS App on the registry; the template's `stale-sweep.yml` then exchanges the run's identity for a GitHub App token that lives for minutes (charter D-10) and opens the PR with it. Nothing in this file changes for the skill: it never opens `stale-source` PRs itself.

## Authorization on push

The instance's `validate.yml` runs `validate.py --changed-only <before> --actor <pusher> --event <event>` on every push to the default branch, after the push has landed. It turns the run red, with one `authorization` line, for a direct push that adds an entry the pusher does not own, edits an entry the pusher did not own before the push (transfer and release by the owner are allowed), deletes an entry, or changes any login but the pusher's own in `adoption.yaml`. It does not reject or revert the push: git has already accepted it, and the red run is the signal to a person to revert. Merges performed by the consensus workflow are exempt; the PR path already gated them.

Two limits follow, and one repository setting backstops both. A red run only blocks the branch when branch protection on the default branch requires the `validate` status check. And the exemption reaches exactly the consensus workflow's actor and `pull_request` events (the workflow passes `github.event_name` as `--event`): a `push` whose `HEAD` is a merge commit is checked like any other, so a local `git merge --no-ff` of an edit to another person's entry, or clicking Merge on one's own PR, turns the run red for whoever pushed or merged. Red after the fact is all the check can do, and the PR's own run is skipped by construction, so requiring the status check alone does not stop a self-merge: branch protection must also require pull requests and at least one approving review. `init` sets up none of these; a repository admin does, once. This is the mechanism that makes the direct-commit column safe to offer at all (charter P-4); the rule's full reach is in `references/adoption.md` and the code in `scripts/validate.py`.
